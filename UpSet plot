...
library(UpSetR)
deg_A <- prom1_TdT_TW_vs_prom1_TdT_sig_genes
deg_B <- prom1_TdT_TW_vs_TdT_sig_genes
deg_C <- prom1_TdT_TW_vs_TdT_TW_sig_genes
deg_D <- prom1_TdT_vs_TdT_sig_genes
deg_E <- TdT_TW_vs_prom1_TdT_sig_genes
deg_F <- TdT_TW_vs_TdT_sig_genes
set_list <- list(
  A = unique(na.omit(deg_A$SYMBOL)),
  B = unique(na.omit(deg_B$SYMBOL)),
  C = unique(na.omit(deg_C$SYMBOL)),
  D = unique(na.omit(deg_D$SYMBOL)),
  E = unique(na.omit(deg_E$SYMBOL)),
  F = unique(na.omit(deg_F$SYMBOL))
)
input_df <- fromList(set_list)
head(input_df)
